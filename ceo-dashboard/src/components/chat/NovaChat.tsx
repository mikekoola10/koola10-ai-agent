...
import { MessageSquare, X, Send, Loader2, Bot, Volume2, Mic } from 'lucide-react';
    ...
    <div className="fixed bottom-6 right-6 z-50">
      {!isOpen ? (
        ...
        <Card className="w-80 sm:w-96 h-[500px] flex flex-col bg-[#1a1033]/95 backdrop-blur-xl border-white/20 shadow-2xl overflow-hidden">
                    ...
                    >
                      ...
                      {m.role === 'nova' && (
                        <button
                          ...
                          className="absolute -right-8 top-1/2 -translate-y-1/2 p-1 text-white/30 hover:text-amber-400 opacity-0 group-hover:opacity-100 transition-opacity"
                        >
          ...
          <CardFooter className="p-4 bg-white/5 border-t border-white/10 shrink-0 flex flex-col gap-2">
            <form onSubmit={handleSend} className="flex w-full gap-2">
              <Input
                placeholder="Type a message..."
                value={input}
                onChange={(e) => setInput(e.target.value)}
                className="bg-black/20 border-white/10 text-white placeholder:text-white/30 grow"
              />
              <Button
                type="button"
                onClick={() => setIsListening(!isListening)}
                className={`px-3 ${isListening ? 'bg-red-500 hover:bg-red-600' : 'bg-white/10 hover:bg-white/20 text-white'}`}
              >
