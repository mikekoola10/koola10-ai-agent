...
var (
	...
	globalSwarm *agents.SwarmManager
)
...
func main() {
	...
	globalSwarm = agents.NewSwarmManager(AddAuditEntry, globalLedger.RecordCost)
	...
	// Accelerated Swarm Endpoints
	http.HandleFunc("/swarm/deploy", corsMiddleware(handleSwarmDeploy))
	http.HandleFunc("/swarm/dispatch", corsMiddleware(handleSwarmDispatch))
...
func handleSwarmStatus(w http.ResponseWriter, r *http.Request) {
	// Support both task-based and general status
	id := r.URL.Query().Get("task_id")
	if id != "" {
		if redisClient == nil {
			http.Error(w, "no redis", 503)
			return
		}
	...
	status := globalSwarm.GetStatus()
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(status)
}

func handleSwarmDeploy(w http.ResponseWriter, r *http.Request) {
	var req struct {
	...
	switch req.Vertical {
	case "trading":
		globalSwarm.DeploySwarms("trading", req.Count, agents.GetTradingFactory(globalSwarm))
	case "leadgen":
		globalSwarm.DeploySwarms("leadgen", req.Count, agents.GetLeadGenFactory(globalSwarm))
	default:
		http.Error(w, "invalid vertical", 400)
		return
	}

	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(map[string]string{"status": "deployed", "vertical": req.Vertical, "count": fmt.Sprint(req.Count)})
}
...
func handleSwarmDispatch(w http.ResponseWriter, r *http.Request) {
	...
	result := globalSwarm.DispatchTask(req.Vertical, req.Task)
	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(map[string]string{"result": result})
}

func handleSwarmRevenue(w http.ResponseWriter, r *http.Request) {
	metrics := globalSwarm.GetAllRevenueMetrics()
	w.Header().Set("Content-Type", "application/json")
...
func handleSwarmReport(w http.ResponseWriter, r *http.Request) {
	...
	report := map[string]interface{}{
		...
		"revenue":   metrics,
		"agents":    status,
		"summary":   "Swarm operational. Trading and LeadGen active.",
	}
