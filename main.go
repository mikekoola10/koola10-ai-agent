...
type StyleRequest struct {
	Description    string `json:"description"`
	GenerateSample bool   `json:"generate_sample"`
	ModelQuery     string `json:"model_query"`
}

type StyleResponse struct {
	StyleRules         string `json:"style_rules"`
	EmergentPrompt     string `json:"emergent_prompt"`
	StyleSampleURL     string `json:"style_sample_url,omitempty"`
	StyleSampleBase64  string `json:"style_sample_base64,omitempty"`
	StyleSampleText    string `json:"style_sample_text,omitempty"`
...
func handleStudioStyle(w http.ResponseWriter, r *http.Request) {
	...
	systemPrompt := "Generate Koola10 style rules (Boondocks + 4K realism) and convert the scene into an Emergent Video prompt. Return JSON with 'style_rules' and 'emergent_prompt'."
	...
	var styleRes StyleResponse
	if err := json.Unmarshal([]byte(dsRes.Choices[0].Message.Content), &styleRes); err != nil {
		http.Error(w, "failed to parse AI response", 500)
		return
	}

	if req.GenerateSample {
		modelQuery := req.ModelQuery
		if modelQuery == "" {
			modelQuery = "text-to-image cinematic 4k"
		}

		// 1. Search for models
		searchResult := tools.RunTool("huggingface", map[string]interface{}{
			"action": "search_models",
			"query":  modelQuery,
		})

		if searchResult.Success {
			models, ok := searchResult.Data.([]interface{})
			if ok && len(models) > 0 {
				firstModel, ok := models[0].(map[string]interface{})
				if ok {
					modelID, _ := firstModel["id"].(string)
					if modelID == "" {
						modelID, _ = firstModel["modelId"].(string)
					}
					if modelID != "" {
						// 2. Run model inference
						runResult := tools.RunTool("huggingface", map[string]interface{}{
							"action": "run_model",
							"model":  modelID,
							"inputs": styleRes.EmergentPrompt,
						})

						if runResult.Success {
							output, _ := runResult.Data.(string)
							// Heuristic to determine output type
							if strings.HasPrefix(output, "http") {
								styleRes.StyleSampleURL = output
							} else if len(output) > 10 { // Base64 is likely long
								styleRes.StyleSampleBase64 = output
							} else {
								styleRes.StyleSampleText = output
							}
						} else {
							styleRes.GenerationError = "Inference failed: " + runResult.Error
						}
					} else {
						styleRes.GenerationError = "No model ID found in search results"
					}
				}
			} else {
				styleRes.GenerationError = "No models found for query: " + modelQuery
			}
		} else {
			styleRes.GenerationError = "Model search failed: " + searchResult.Error
		}
	}

	w.Header().Set("Content-Type", "application/json")
	json.NewEncoder(w).Encode(styleRes)
}
