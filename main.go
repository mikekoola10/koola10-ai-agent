...
func main() {
	...
	http.HandleFunc("/stripe/webhook", handleStripeWebhook) // Webhook usually handles its own security/validation
...
func (l *EconomicLedger) RecordRevenue(amount float64, source string) {
	...
	l.Transactions = append(l.Transactions, Transaction{time.Now().Format(time.RFC3339), "revenue", "revenue_split", amount, "Revenue: " + source})
