...
func handleStudioLore(w http.ResponseWriter, r *http.Request) {
	...
	hReq, _ := http.NewRequest("POST", getDeepSeekBaseURL()+"/chat/completions", bytes.NewBuffer(dsBody))
...
func getDeepSeekBaseURL() string {
	baseURLMu.RLock()
	if time.Since(lastBaseURLCheck) < 1*time.Minute && cachedBaseURL != "" {
		url := cachedBaseURL
		baseURLMu.RUnlock()
		return url
	}
	...
	// Re-check because another goroutine might have updated it
	if time.Since(lastBaseURLCheck) < 1*time.Minute && cachedBaseURL != "" {
	...
	if err == nil {
		resp.Body.Close()
		cachedBaseURL = "http://localhost:11434/v1"
	} else {
...
func handleApply(w http.ResponseWriter, r *http.Request) {
	...
	dsBody, _ := json.Marshal(dsReq); hReq, _ := http.NewRequest("POST", getDeepSeekBaseURL()+"/chat/completions", bytes.NewBuffer(dsBody))
...
func handleSpecialistSwarm(w http.ResponseWriter, r *http.Request) {
	...
	case "POST":
		if action == "deploy" {
			...
			json.NewDecoder(r.Body).Decode(&req)
			res, err := globalSwarmManager.DispatchTask(vertical, req.Task)
			if err != nil {
				http.Error(w, err.Error(), 500)
				return
			}
