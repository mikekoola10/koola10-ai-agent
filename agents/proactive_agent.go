...
func (pa *ProactiveAgent) checkTriggers() {
	...
	if !lastInteraction.IsZero() && now.Sub(lastInteraction) > 4*time.Hour {
		pa.Broadcast("speak", "Hey there, I haven't heard from you in a while. Just checking in to see if you need anything!", "Sterling")
	...
	if (hour == 9 && minute < 15) {
		pa.Broadcast("speak", "Good morning! I'm ready for our daily briefing. We have several pending grant opportunities and the trading pool is performing well.", "Sterling")
	} else if (hour == 18 && minute < 15) {
		pa.Broadcast("speak", "Good evening. Wrapping up the day's work. The lead generation swarm found 10 new high-quality leads today.", "Nova")
	}
}
